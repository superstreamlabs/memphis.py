...
class Memphis:
    ...
    async def get_msgs_update_configurations(self, iterable: Iterable) -> None:
        try:
    ...
    async def send_notification(self, title: str, msg, failedMsg, type):
    ...
    async def station(
        self,
        name,
        retention_type: str = retention_types.MAX_MESSAGE_AGE_SECONDS,  # str -> enum
        retention_value: int = 604800,
        storage_type: str = storage_types.DISK,  # str -> enum
        replicas: int = 1,
        ...
        try:
            ...
            createStationReq = {
                ...
                "dls_configuration": {"poison": send_poison_msg_to_dls, "Schemaverse": send_schema_failed_msg_to_dls},
    ...
    async def attach_schema(self, name: str, stationName: str):
    ...
    async def producer(self, station_name: str, producer_name: str, generate_random_suffix=False):
        """Creates a producer.
        Args:
    ...
    def parse_descriptor(self, station_name: str):
        try:
            ...
            proto_msg = MessageFactory(pool).GetPrototype(pool.FindMessageTypeByName(msg_name))
            proto = proto_msg()
    ...
    async def start_listen_for_schema_updates(self, station_name: str, schema_update_data):
        ...
        if not task_exists:
            ...
            task = loop.create_task(
                self.get_msg_schema_updates(station_name, self.schema_updates_subs[station_name].messages)
            )
                ...
                if "Schema validation has failed" in str(e) or "Unsupported message type" in str(e):
                    ...
                    if self.connection.station_schemaverse_to_dls[self.internal_station_name]:
                        ...
                        buf = {
                            ...
                            "producer": {"name": self.producer_name, "connection_id": self.connection.connection_id},
                        ...
                        await self.connection.broker_connection.publish(
                            "$memphis-" + self.internal_station_name + "-dls.schema." + id, buf
                        )
...
class Consumer:
    ...
    async def __consume(self, callback):
        ...
        self.psub = await self.connection.broker_connection.pull_subscribe(subject + ".final", durable=consumer_group)
        while True:
